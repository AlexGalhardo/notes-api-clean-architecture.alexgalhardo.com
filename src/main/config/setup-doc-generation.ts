...
function createMorganTokens() {
    morgan.token("resBodySchema", function (req, res: any) {
        ...
        const responseBodySchema = Object.keys(responseBody)
            .map((key) => {
    ...
    morgan.token("reqBodySchema", (req: Request, res) => {
        ...
        const lineBreakAndTabs = "\n\t\t\t\t\t\t\t\t\t\t\t";
        const reqBodySchema = Object.keys(reqBody)
            .map((key) => {
    ...
    morgan.token(
        "randomId",
        (req: Request, res) => Math.random().toString(36).substring(2, 5) + Math.random().toString(36).substring(2, 5),
    );
